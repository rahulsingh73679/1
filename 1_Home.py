...
def init_database():
    ...
    if db_exists:
        try:
            # Check if database is valid and has tables
    ...
    if not db_exists or not db_valid:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                st.warning("Database initialization completed with warnings.")
        except FileNotFoundError:
            st.error("setup_database.py not found. Please ensure it's in the repository.")
        except subprocess.CalledProcessError as e:
            st.error(f"Error initializing database: {e.stderr}")
...
def main():
    ...
    if not subjects:
        st.warning("No subjects found in the database. Please run setup_database.py to initialize the database.")
        st.stop()
    ...
    if not papers:
        st.warning(f"No papers found for {selected_subject}. Please add papers to the database.")
        st.stop()
    ...
    if not questions:
        st.warning(f"No questions found for the selected paper. Please add questions to the database.")
        st.stop()
    qnum = 1
    optnum = 1
